...
router.get("/assembly-group-oee-apq", async (req, res) => {
  try {
    const {
      groupNo,
      filterType,
    ...
    const result = await request.execute("SP_AssemblyGroup_OEE_APQ");

    res.json(result.recordset);
  } catch (err) {
    console.error("Error in GET /assembly-group-oee-apq", err);
